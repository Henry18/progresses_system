@php
    $trx = \App\Models\Transaction::with('user')->latest()->limit(10)->get();

    $fakeTransaction = \App\Models\Frontend::where('data_keys', 'transaction.element')->limit(10)->get();
                        ...
                        @foreach ($transactions as $data)
                            <tr>
                                @if (@$data->data_values)
                                    ...
                                    <td>{{ is_numeric(@$data->data_values->amount) ? showAmount(@$data->data_values->amount) : @$data->data_values->amount }}</td>
